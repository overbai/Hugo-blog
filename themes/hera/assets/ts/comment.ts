...
export class farallonComment extends farallonHelper {
    ...
    constructor(config: farallonCommentOptions) {
        super();
        this.postSelector = config.postSelector || ".hComment--area";
    ...
    renderComment(item: Comment, children: any = "", reply: boolean = true) {
        const replyHtml: string = reply
            ? `<span class="comment-reply-link u-cursorPointer" onclick="return addComment.moveForm('comment-${
                  ...
                  (document.querySelector(".hComment--area") as HTMLElement)!
                      .dataset.id
            ...
            : "";
        return `<li class="comment parent hComment--item" itemtype="http://schema.org/Comment" data-id="${item.comment_ID}" itemscope="" itemprop="comment" id="comment-${item.comment_ID}">
                            <div class="hComment--body">
                                <div class="hComment--header">
                                    ...
                                    <div class="hComment--meta">
                                        ${item.comment_author}${replyHtml}
                                            <div class="hComment--time" itemprop="datePublished" datetime="${item.comment_date}">${item.comment_date}</div>
                                    </div>
                                </div>
                                <div class="hComment--content" itemprop="description">
        ...
        fetch(
            ...
            res.json().then((data) => {
                ...
                document.querySelector(
                    ".hComment--heroTitle .count"
                )!.innerHTML = this.total.toString();
                if (this.total == 0) {
                    document.querySelector(
                        ".hComment--list"
                    )!.innerHTML = `<div class="hComment--placeholder">${window.noComment}</div>`;
                    return;
                } else {
                    const html = comments
                        .map((item: Comment) => {
                        ...
                        .join("");
                    document.querySelector(".hComment--list")!.innerHTML = html;
                }
            });
        });
    }
                    ...
                    fetch(this.actionDomain + "comment/insert", {
                        ...
                        .then((data) => {
                            ...
                            if (parent_id != "") {
                                ...
                                if (
                                    document.querySelector(
                                        ".hComment--placeholder"
                                    )
                                ) {
                                    document
                                        .querySelector(".hComment--placeholder")
                                        ?.remove();
                                }
                                document
